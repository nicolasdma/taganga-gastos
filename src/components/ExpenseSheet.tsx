...
function AmountStep({
  ...
  const scope: ExpenseScope = view
  const [saving, setSaving] = useState(false)

  const saveWithItem = async (expenseAmount: number) => {
    setSaving(true)
    await saveExpense({
...
function ExpenseSheetBody({
  ...
  const sheetTitle = (() => {
    ...
    if (step === 'amount' && selectedItem) {
      return `${selectedItem.itemEmoji} ${selectedItem.itemLabel}`
    }
    return 'Monto'
  })()
